...
test('立绘系统提示词要求输出中文自然语句并覆盖固定结构', () => {
  ...
  assert.match(ILLUSTRATOR_SYSTEM_PROMPT, /人物主体：女性、年龄、职业、种族/);
  assert.match(ILLUSTRATOR_SYSTEM_PROMPT, /人物主体部分不得出现稀有度/);
  assert.doesNotMatch(ILLUSTRATOR_SYSTEM_PROMPT, /人物主体：[^。\n]*稀有度。/);
...
test('本地立绘提示词按新结构输出主体、风格、必须提示词、镜头、服饰、武器、背景、特效和动作', () => {
  ...
  assert.doesNotMatch(prompt.split('，整体为')[0], /稀有度|SR|SSR|UR|R/);
